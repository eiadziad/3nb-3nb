...
client.on('message', (channel, userstate, message, self, viewers) => {
  ...
  const txb_r = "txb_r"; // اسم المستخدم الذي يمكنه استخدام أمر !حدث

client.on('message', (channel, userstate, message, self) => {
  // تجاهل الرسائل الصادرة عن البوت نفسه
  if (self) return;

  if (message.includes('!حدث') && userstate.username.toLowerCase() === txb_r.toLowerCase()) {
    usersList.length = 0; // يفرغ القائمة بشكل كامل
    client.say(channel, `@${txb_r} تم تحديث القائمة بنجاح.`);
...
const wordsList2 = [
  'NARUTO', 'BLEACH', 'ONE PIECE', 'DRAGON BALL', 'ATTACKONTITAN',
  'DEATHNOTE', 'MY HERO ACADEMIA', 'TOKYOGHOUL', 'FULLMETAL ALCHEMY', 'SWORDARTONLINE',
  'NEON GENESIS EVANGELION', 'FAIRYTAIL', 'POKEMON', 'ONEPUNCHMAN', 'HUNTER X HUNTER',
  'COWBOYBEBOP', 'BLACK CLOVER', 'DEMON SLAYER', 'BLENDERS', 'JOJOS BIZARRE ADVENTURE',
...
const startTime2 = moment.tz('2024-01-16 10:29:12', 'Asia/Riyadh'); // تعيين الوقت المحدد بتوقيت الرياض
...
client.on('message', (channel, userstate, message, self) => {
  ...
  if (
    ...
    let timePassed2 = `${days} أيام و ${hours} ساعة و ${minutes} دقيقة و ${seconds} ثانية`;
...
client.on('message', (channel, userstate, message, self) => {
  // تحقق من أن الرسالة ليست من البوت نفسه وأنها تحتوي على الأمر المناسب
  if (self || !message.startsWith('!احسب')) {
      return;
  }

  // فصل الأمر والقيمة من الرسالة
  const [, amount, currencyType] = message.split(' ');

  // التحقق من وجود القيمة
  if (!amount || !currencyType) {
      client.say(channel, 'ارجو تحديد القيمة ثم نوع العملة (دولار، بتس، تير1، تير2، تير3)');
      return;
  }

  // حساب الزمن وإرسال الرد
  const timeInSeconds = calculateTime(parseInt(amount), currencyType);
  client.say(channel, timeInSeconds);
});
...
function calculateTime(amount, currencyType) {
  ...
  return `الوقت المقابل لـ ${amount} ${currencyType} هو ${timeInSeconds} ثانية`;
}
...
client.on('message', (channel, userstate, message, self) => {
  ...
  if (message.startsWith('!clc')) {
      ...
      const operation = message.substring(6);

      // التحقق من الصحة قبل استخدام eval()
      if (!isValidMathOperation(operation)) {
          client.say(channel, `العملية الرياضية غير صحيحة.`);
          return;
      }

      try {
          // حساب العملية الرياضية وإرسال النتيجة
          const result = eval(operation);
          client.say(channel, `equals: ${result}`);
      } catch (error) {
...
// التحقق من صحة العملية الرياضية
function isValidMathOperation(operation) {
  const regex = /^[0-9+\-*/().\s]+$/;
